...
import { PURCHASES_BY_UID_QUERY, useGetCurrentUserQuery, UPDATE_PURCHASE } from "@/service/vaxsul";
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {orders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td className="px-6 py-4 text-center text-sm text-gray-500">{order.vaccine?.name}</td>
                  <td className="px-6 py-4 text-center text-sm text-gray-500">{order.amount}</td>
                  <td className="px-6 py-4 text-center text-sm text-gray-500">{order.user?.name}</td>
                  <td className="px-6 py-4 text-center text-sm text-gray-500">R$ {order.totalSpent.toFixed(2)}</td>
                  <td className="px-6 py-4 text-center text-sm text-gray-500">{PURCHASE_STATUS_TEXT[order.status]}</td>
                </tr>
              ))}
            </tbody>
          </table>
